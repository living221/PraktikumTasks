package YandexPraktikumTasks.JavaCore1.sprint_7.TimeAndDate.task10;
...
class Practicum {
    ...
    private static void printFlightInformation(
        ...
        ZonedDateTime arrival = departure.plus(flightDuration).withZoneSameLocal(ZoneId.of(arrivalAirport.getZone()));
        ...
        // Только время вылета
        String departureTimeOnly = arrival.format(TIME_FORMATTER);
        ...
        if (delay > 0) {
            ...
            ZonedDateTime departureWithDelay = departure.plusMinutes(delayDuration.toMinutes());
            // Вычислите время прилёта с учётом задержки.
            ZonedDateTime arrivalWithDelay = arrival.plusMinutes(delayDuration.toMinutes());
            ...
            System.out.println("Обновлённое время вылета: "
                    + departureWithDelay.getHour()
                    + ":"
                    + departureWithDelay.getMinute()
            );
            // Выведите отформатированное время прилёта с учётом задержки.
            System.out.println("Обновлённое время прилёта: "
                    + arrivalWithDelay.getHour()
                    + ":"
                    + arrivalWithDelay.getMinute()
            );
        } else {
