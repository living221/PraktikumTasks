package YandexPraktikumTasks.JavaCore1.sprint_5.list.task07;
...
public class PotatoLaboratory {
    ...
    private static List<Potato> findPotatoesForExperiment(List<Potato> potatoes) {
        ...
        ArrayList<Potato> potatoesMutable = new ArrayList<>(potatoes);
        List<Potato> minAndMaxAlphaPotatoes = new ArrayList<>(List.of(Collections.min(potatoesMutable), Collections.max(potatoesMutable)));
        potatoesMutable.removeAll(minAndMaxAlphaPotatoes);
        minAndMaxAlphaPotatoes.add(Collections.min(potatoesMutable));
        minAndMaxAlphaPotatoes.add(Collections.max(potatoesMutable));
        Collections.sort(minAndMaxAlphaPotatoes);
        return minAndMaxAlphaPotatoes;
    }
}
