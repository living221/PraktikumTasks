package YandexPraktikumTasks.JavaCore1.sprint_5.hashtables.task10;
...
public class Practicum {
    ...
    private static void printOrderedByDateReceived(Set<Letter> letters) {
        ...
        Set<Letter> orderedByDateLetters = new TreeSet<>(Comparator.comparing(l -> l.dateReceived));
        orderedByDateLetters.addAll(letters);

        for (Letter letter : orderedByDateLetters) {
            System.out.println("    * Письмо от " + letter.authorName + " поступило " + letter.dateReceived);
        }
    }
