package YandexPraktikumTasks.JavaCore1.sprint_6.files.task05;
...
public class KeywordsGame {
    ...
    public void runGame() {
        final int rnd = random.nextInt(KEYWORDS.size());
        int incorrectAnswersCounter = 0;
        while (true) {
            final boolean providedAnswer;
            if (rnd < RESERVED.size()) {
    ...
    private boolean writeReservedQuestion() {
        ...
        final int rnd = random.nextInt(RESERVED.size());
        System.out.println(RESERVED.get(rnd));
        return true;
    }
